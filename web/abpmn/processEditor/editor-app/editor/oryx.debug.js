...
// );
//		
// if(urls.length == 0) { throw 'The resource with the id ' +id+ ' has no
// process url.'};
//		
 ...
 * 
 * @example var MyClass = Clazz.extend({ construct: function(myParam){ // Do
 *          sth. } }); var MySubClass = MyClass.extend({ construct:
 *          function(myParam){ // Use this to call constructor of super class
 *          arguments.callee.$.construct.apply(this, arguments); // Do sth. }
 *          });
 * @param {Object}
...
ORYX.CONFIG.KEY_CODE_DOWN =				40;
	...
	// TODO Determine where the lowercase constants are still used and remove
	// them from here.
...
ORYX = Object.extend(ORYX, {
	 ...
	 *  // point out how many attempts are left... ORYX.Log.debug("Waiting once
	 * more (%0 attempts left)", (ORYX_CONFIGURATION_WAIT_ATTEMPTS -
			...
			onSuccess: function(result) {
				...
				preferences.each( function(p) {
					...
					props.each( function(prop) {
						...
						// get all attributes from the node and set to global
						// properties
...
ORYX.Core.SVG.MinMaxPathHandler = Clazz.extend({
	...
	/**
	 * Store minimal and maximal coordinates of passed points to attributes
	 * minX, maxX, minY, maxY
	 * 
	 * @param {Array}
	 *            points Array of absolutePoints
				...
				if (this.node.ownerDocument) {
					...
					if (this.fitToElemId || this._textHasChanged){
						...
						textLines.each((function(textLine, index){
							...
							/*
							 * Chrome's getBBox() method fails, if a text node
							 * contains an empty tspan element. So, we add a
							 * whitespace to such a tspan element.
							 */
...
// console.log(tspan.textContent.charAt(i), tspan.getSubStringLength(i,1),
...
// }
	...
	connectMorph: function(args) {	
		...
		var edgeStencils = incoming.select(function(e) { return outgoing.member(e); }); // intersection
																						// of
																						// sets
		var baseEdgeStencils = this.baseMorphs().select(function(e) { return edgeStencils.member(e); }); // again:
																											// intersection
																											// of
																											// sets
...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
    ...
    construct: function(source, modelMetaData, editorId){
		...
		if(ORYX.CONFIG.BACKEND_SWITCH) {
			...
			new Ajax.Request(ACTIVITI.CONFIG.contextRoot + 'abpmn/stencilsetRestResourceAction!getStencilset.do?version=' + Date.now(), {
	            asynchronous: false,
	            method: 'get',
...
ORYX.Core.UIObject = {
	...
	construct: function(options) {	
		...
		this.isChanged = true;			// Flag, if UIObject has been changed
										// since last update.
		this.isResized = true;
		this.isVisible = true;			// Flag, if UIObject's display attribute
										// is set to 'inherit' or 'none'
		...
		this.node = undefined;			// this is a reference to the SVG
										// representation, either locally or in
										// DOM.
		this.children = [];				// array for all add uiObjects
		
		this.bounds = new ORYX.Core.Bounds();		// bounds with undefined
													// values
 ...
 * @example jQuery.extend(shapeAsJson, ORYX.Core.AbstractShape.JSONHelper);
 */
ORYX.Core.AbstractShape.JSONHelper = {
     /**
		 ...
		 *            parent as arguments.
		 * @param {boolean}
		 *            [deep=false] Iterate recursively (childShapes of
		 *            childShapes)
		 * @param {boolean}
		 *            [modify=false] If true, the result of the iterator
		 *            function is taken as new shape, return false to delete it.
		 *            This enables modifying the object while iterating through
		 *            the child shapes.
		 * @example // Increases the lowerRight x value of each direct child
		 *          shape by one. myShapeAsJson.eachChild(function(shape,
		 *          parentShape){ shape.bounds.lowerRight.x =
		 *          shape.bounds.lowerRight.x + 1; return shape; }, false,
		 *          true);
		 */
     eachChild: function(iterator, deep, modify){
         ...
         this.childShapes.each(function(shape){
             ...
             if(res) newChildShapes.push(res); // if false is returned, and
												// modify = true, current shape
												// is deleted.
 ...
 */	
	 ...
	 * to the canvas.
	 * 
	 * @example canvas.addShapeObjects({ bounds:{ lowerRight:{ y:510, x:633 },
	 *          upperLeft:{ y:146, x:210 } },
	 *          resourceId:"oryx_F0715955-50F2-403D-9851-C08CFE70F8BD",
	 *          childShapes:[], properties:{}, stencil:{ id:"Subprocess" },
	 *          outgoing:[{resourceId: 'aShape'}], target: {resourceId:
	 *          'aShape'} });
	 * @param {Object}
	 *            shapeObjects
	 * @param {Function}
...
ORYX.Editor = {
				...
				ORYX.Log.debug("Initializing plugin '%0'", match.name);
					...
					try {
						...
						var className 	= eval(match.name);
							...
							// If there have an onSelection-Method it will
							// pushed to the Editor Event-Handler
	...
	loadPlugins: function() {
		...
		newPlugins.each(function(value) {
			...
			// If there have an onSelection-Method it will pushed to the Editor
			// Event-Handler
	 ...
	 * 
	 * @example editor.loadSerialized({ resourceId: "mymodel1", childShapes: [ {
	 *          stencil:{ id:"Subprocess" }, outgoing:[{resourceId: 'aShape'}],
	 *          target: {resourceId: 'aShape'}, bounds:{ lowerRight:{ y:510,
	 *          x:633 }, upperLeft:{ y:146, x:210 } }, resourceId: "myshape1",
	 *          childShapes:[], properties:{}, } ], properties:{ language:
	 *          "English" }, stencilset:{
	...
	_handleMouseDown: function(event, uiObj) {
		...
		// Rule #8: When the element is selectable and is currently selected and
		// no
		...
		if(this._dockedShape) {
			...
			var dock2 = this.parent.dockers.length > 1 ? 
							(dockerIndex === 0?							// If
																		...
																		// first
																		// element
							 	this.parent.dockers[dockerIndex + 1]:	// then
																		// take
																		// the
																		// next
																		// docker
								this.parent.dockers[dockerIndex - 1]):  // if
																		// not,
																		// then
																		// take
																		// the
			 ...
			 */
				...
				// Get the new point for the Docker, calucalted by the
				// intersection point of the Shape and the two points
				newPoint = this._dockedShape.getIntersectionPoint(absoluteReferenzPoint1, absoluteReferenzPoint2);
				
				// If there is new point, take the referencepoint as the new
				// point
				if(!newPoint) {
...
ORYX.Core.Controls.Magnet = ORYX.Core.Controls.Control.extend({
	...
	refresh: function() {
		...
		var p = this.bounds.upperLeft();
		/*
		 * if(this.parent) { var parentPos = this.parent.bounds.upperLeft(); p.x +=
		 * parentPos.x; p.y += parentPos.y; }
		 */
    ...
    deserialize: function(data){
		...
		outgoing.each((function(obj){
			...
			if(next){
				...
				} else if(next.dockers.length > 0) { // next is a node and
														// next has a docker
					next.dockers.first().setDockedShape(this);
					// next.dockers.first().setReferencePoint({x:
					// this.bounds.width() / 2.0, y: this.bounds.height() /
					// 2.0});
				}
			}	
    ...
    _update: function(force){
        if(this._dockerUpdated || this.isChanged || force) {
	        ...
	        this.dockers.each((function(docker){
	            ...
	            // If there is any changes at the edge and is there is not an
				// DockersUpdate
			...
			this.getLabels().each(function(label) {
				...
				if (label.getReferencePoint()){
					...
					if (!this.parent.initializingShapes) {
						...
						oiv.x *= ref.distance; oiv.y *= ref.distance; 				// vector
																					// *
																					// distance
						oiv.x += newIntersection.x; oiv.y += newIntersection.y; 	// vector
																					// +
																					// the
																					// intersection
																					// point
						...
						oiv.x *= ref.distance; oiv.y *= ref.distance; // vector
																		// *
																		// distance
						oiv.x += ref.intersection.x; oiv.y += ref.intersection.y; // vector
																					// +
																					// the
																					// intersection
																					// point
		...
		this.getLabels().each(function(label){
			...
			if (index >= ref.segment.fromIndex && index <= ref.segment.toIndex){
				...
				if (!segment){ 
					...
					segment.toDocker = this.dockers[this.dockers.indexOf(from)+1]; // The
																					// next
																					// one
																					// if
																					// the
				...
				var intersection = ORYX.Core.Math.getPointOfIntersectionPointLine(
										fromPosition, 		// P1 - Center of
															// the first docker
										toPosition, 		// P2 - Center of
															// the second docker
										ref.intersection, 	// P3 - Center of
															// the label
										true);
				// var oldDistance =
 ...
 * this.dockers.last().getDockedShape(); var sharedParent; if (source && target) {
 * //get shared parent while (source.parent) { source = source.parent; if
 * (source instanceof ORYX.Core.Canvas) { sharedParent = source; break; } else {
 * var targetParent = target.parent; var found; while (targetParent) { if
 * (source === targetParent) { sharedParent = source; found = true; break; }
 * else { targetParent = targetParent.parent; } } if (found) { break; } } } }
 * else if (source) { sharedParent = source.parent; } else if (target) {
 ...
 * 
 * @example ORYX.Plugins.MyPlugin = ORYX.Plugins.AbstractPlugin.extend({
 *          construct: function() { // Call super class constructor
 *          arguments.callee.$.construct.apply(this, arguments);
 * 
 * [...] }, [...] });
 * 
...
ORYX.Plugins.AbstractPlugin = Clazz.extend({
	 ...
	 * @example openDownloadWindow( "my.xml", "<exampleXML />" );
	 */
...
ORYX.Plugins.AbstractLayouter = ORYX.Plugins.AbstractPlugin.extend({
	...
	/**
	 * 'layouted' defined all types of shapes which will be layouted. It can be
	 * one value or an array of values. The value can be a Stencil ID (as
	 * String) or an class type of either a ORYX.Core.Node or ORYX.Core.Edge
	 * 
    ...
    getAllShapesToConsider: function(shapes){
        ...
        var edgesToConsider = this.facade.getCanvas().getChildEdges().select(function(edge){
            ...
            return edge.getAllDockedShapes().all(function(shape){
                // Remember: Edges can have other edges on outgoing, that is why
				// edges must not be included in childShapesToConsider
...
ORYX.Plugins.ProcessLink = Clazz.extend({
	 ...
	 * ['path', { "style": "fill:none;stroke-width:0.5px; stroke:#000000", "d":
	 * "M4,8 l-2,0 l0,6"}], ['path', { "style": "fill:none;stroke-width:0.5px;
	 * stroke:#000000", "d": "M10,8 l2,0 l0,6"}], ['rect', { "style":
	 * "fill:#96ff96;stroke:#000000;stroke-width:1", "width": 6, "height": 4,
	 * "x": 4, "y": 0}], ['rect', { "style":
	 * "fill:#ffafff;stroke:#000000;stroke-width:1", "width": 6, "height": 4,
	 * "x": 4, "y": 6}], ['rect', { "style":
	 * "fill:#96ff96;stroke:#000000;stroke-width:1", "width": 6, "height": 4,
	 * "x": 0, "y": 12}], ['rect', { "style":
	 * "fill:#96ff96;stroke:#000000;stroke-width:1", "width": 6, "height": 4,
	 * "x": 8, "y": 12}], ['rect', { "style":
	 * "fill:none;stroke:none;pointer-events:all", "width": 14, "height": 16,
	 * "x": 0, "y": 0}] ]);
	 */
...
ORYX.Plugins.DragDropResize = ORYX.Plugins.AbstractPlugin.extend({
	...
	construct: function(facade) {
		...
		this.distPoints 		= [];			// Distance Points for Snap on
												// Grid
		this.isResizing 		= false;		// Flag: If there was currently
												// resized
		this.dragEnable 		= false;		// Flag: If Dragging is enabled
		this.dragIntialized 	= false;		// Flag: If the Dragging is
												// initialized
		this.edgesMovable		= true;			// Flag: If an edge is docked it
												// is not movable
		this.offSetPosition 	= {x: 0, y: 0};	// Offset of the Dragging
		this.faktorXY 			= {x: 1, y: 1};	// The Current Zoom-Faktor
		this.containmentParentNode;				// the current future parent
												// node for the dragged shapes
		this.isAddingAllowed 	= false;		// flag, if adding current
												// selected shapes to
												// containmentParentNode is
												// allowed
		this.isAttachingAllowed = false;		// flag, if attaching to the
												// current shape is allowed
...
// }
			...
			if(ORYX.CONFIG.GRID_ENABLED) {
				...
				this.distPointTimeout = window.setTimeout(function(){
					...
					// Consider only those elements who shares the same parent
					// element
					...
					distShapes.each((function(value) {
						if(!(value instanceof ORYX.Core.Edge)) {
							...
							// Add the upperLeft, center and lowerRight - Point
							// to the distancePoints
	...
	snapToGrid: function(position) {
		...
		this.distPoints.each(function(value) {
			...
			if (Math.abs(value.c.x-c.x) < cThres){
				 ...
				 * (Math.abs(value.lr.x-lr.x) < lrThres){ x = value.lr.x-lr.x;
				 * gx = value.lr.x; }
				 */
			...
			if (Math.abs(value.c.y-c.y) < cThres){
				 ...
				 * (Math.abs(value.lr.y-lr.y) < lrThres){ y = value.lr.y-lr.y;
				 * gy = value.lr.y; }
				 */
		...
		for(var i=0; i<this.moveShapes.length ;i++){
			...
			if (value instanceof ORYX.Core.Node) {
				...
				var allEdges = [].concat(value.getIncomingShapes())
					...
					// Remove all edges which are included in the selection from
					// the list
					...
					// Remove all edges which are between the node and a node
					// contained in the selection from the list
	...
	dockerMoved: function(event) {
		...
		if (this.docker.parent) {
			if (this.isStartDocker || this.isEndDocker) {
				...
				// If the top level uiObj instance of Shape and this isn't the
				// parent of the docker
				}
				else 
					if (uiObj instanceof ORYX.Core.Shape) {
						...
						if (this.docker.parent instanceof ORYX.Core.Edge) {
							...
							/*
							 * Ensure that the shape to dock is not a child
							 * shape of the same edge.
							 */
							...
							while (!this.isValid && curObj && !(curObj instanceof ORYX.Core.Canvas))
							{
								...
								this.isValid = this.facade.getRules().canConnect({
											sourceShape: this.dockerSource ? // Is
																				// there
																				// a
																				// docked
																				// source
															this.dockerSource : // than
																				// set
																				// this
															(this.isStartDocker ? // if
																					// not
																					// and
																					// if
																					// the
											...
											targetShape: this.dockerTarget ? // Is
																				// there
																				// a
																				// docked
																				// target
						...
						// If there is no top level Shape, then hide the
						// highligting of the last Shape
						this.hideHighlight();
...
ORYX.Plugins.KeysMove = ORYX.Plugins.AbstractPlugin.extend({
	...
	move: function(key, far, e) {
		...
		selection = selection.map(function(shape){ 
			if( shape instanceof ORYX.Core.Node ){
				/*
				 * if( shape.dockers.length == 1 ){ return shape.dockers.first() }
				 * else {
				 */
					return shape
				// }
...
new function(){
	...
	ORYX.Plugins.BPMN2_0 = {
                ...
                if(selection && selection.length === 1) {
                        ...
                        if(shape.getStencil().idWithoutNs() === "Pool") {
                                if(shape.getChildNodes().length === 0) {
                                        // create a lane inside the selected
										// pool
                                        var option = {
                                                        type:namespace + "Lane",
                                                        position:{x:0,y:0},
                        ...
                        if (pool&&pool.parent){        
                                ...
                                if (isLeaf && parentHasMoreLanes){
                                        ...
                                        this.facade.executeCommands([command]);
                                                        ...
                                                        !this.facade.getSelection().any(function(select){ // Find
																											// one
																											// of
																											// the
																											// selection,
        ...
        handleLayoutPool: function(event){
                ...
                // Check if the lane is within the pool and is not removed
				// lately
				 ...
				 */            
                        ...
                        // Check if something has changed and maybe create a
						// command
                ...
                while (++i < allLanes.length) {
                        ...
                        // Cache also the bounds of child shapes, mainly for
						// child subprocesses
        ...
        adjustHeight: function(lanes, changedLane, propagateHeight){
                ...
                while (++i<lanes.length){
                        ...
                        if (lanes[i] === changedLane) {
                                ...
                                // Set height propotional to the propagated and
								// old height
        ...
        getOffset: function(lane, includePool, pool){
				 ...
				 * undefined); if (offParent){ var ul =
				 * parent.bounds.upperLeft(); var ulo = offParent.upperLeft();
				 * offset.x += ul.x-ulo.x; offset.y += ul.y-ulo.y; }
				 * 
                ...
                while (++i < lanes.length) {
                        ...
                        while (++j < children.length) {
                                ...
                                var edges = [].concat(children[j].getIncomingShapes())
                                        .concat(children[j].getOutgoingShapes())
                                        // Remove all edges which are included
										// in the selection from the list
                                        .findAll(function(r){ return r instanceof ORYX.Core.Edge })
                                ...
                                while (++k < edges.length) {                    
                                        ...
                                        while (++l < edges[k].dockers.length) {
                                                ...
                                                // Check if the modified center
												// included the new position
                                                var isOverLane = oldBounds.isIncluded(pos);
                                                // Check if the original center
												// is over the pool
                                                ...
                                                // Check if the previous dockers
												// docked shape is from this
												// lane
                                                // Otherwise, check if the
												// docker is over the lane OR is
												// outside the lane
                                                // but the previous/next was
												// over this lane
                                                if (isOverLane){
												 ...
												 * edges[k].dockers[l-1].isDocked()){
												 * var dockedLane =
												 * this.getNextLane(edges[k].dockers[l-1].getDockedShape());
												 * if (dockedLane != lanes[i])
												 * continue; dockers[docker.id] =
												 * {docker: docker,
												 * offset:offset}; } // Check if
												 * the next dockers docked shape
												 * is from this lane else if (l ==
												 * edges[k].dockers.length-2 &&
												 * edges[k].dockers.length>2 &&
												 * edges[k].dockers[l+1].isDocked()){
												 * var dockedLane =
												 ...
												 * dockers[docker.id] = {docker:
												 * docker,
												 * offset:this.getOffset(lanes[i],
												 * true, pool)}; }
												 */
        ...
        getLanes: function(shape, recursive){
                ...
                lanes = lanes.sort(function(a, b){
                                        ...
                                        // Get y coordinates for upper left and
										// lower right
	...
	var ResizeLanesCommand = ORYX.Core.Command.extend({
        ...
        construct: function(shape, parent, pool, plugin) {
				 ...
				 * this.shapeUpperLeft.x += pAbs.x; this.shapeUpperLeft.y +=
				 * pAbs.y; }
				 */
        ...
        findNewLane: function(){
				 ...
				 * a.absoluteXY().y; var bupl = b.absoluteXY().y; return aupl <
				 * bupl ? -1 : (aupl > bupl ? 1 : 0) })
				 */
        ...
        execute: function() {
                ...
                /*
				 * Rescue all ChildShapes of the deleted Shape into the lane
