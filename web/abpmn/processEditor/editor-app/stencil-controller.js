...
angular.module('activitiModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
        ...
        // Code that is dependent on an initialised Editor is wrapped in a
		// promise for the editor
        $scope.editorFactory.promise.then(function () {
            ...
            $http({method: 'GET', url: KISBPM.URL.getStencilSet()}).success(function (data, status, headers, config) {
                ...
                {
                	// Check if the root group is the 'diagram' group. If so,
                    ...
                    if (!removed) {
                        // Check if this group already exists. If not, we create
						// a new one

                        if (currentGroupName !== null && currentGroupName !== undefined && currentGroupName.length > 0) {

                            currentGroup = findGroup(currentGroupName, stencilItemGroups); // Find
																							// group
																							// in
																							// root
																							// groups
                            ...
                            for (var groupIndex = 1; groupIndex < data.stencils[stencilIndex].groups.length; groupIndex++) {
                                ...
                                // The current group variable holds the parent
								// of the next group (if any),
                                // and is basically the last element in the
								// array of groups defined in the stencil item
                ...
                // remove quick menu items which are not available anymore due
				// to custom pallette
            ...
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    {
                    	// ignore canvas event because of empty selection when
						// scrolling stops
                    	return;
                    }
                    ...
                    // Only do something if another element is selected (Oryx
					// fires this event multiple times)
                    if ($scope.selectedShape !== undefined && $scope.selectedShape.getId() === selectedShape.getId()) {
                        if ($rootScope.forceSelectionRefresh) {
                            // Switch the flag again, this run will force
							// refresh
                            $rootScope.forceSelectionRefresh = false;
                        } else {
                            // Selected the same element again, no need to
							// update anything
                            return;
                        }
                    }
                    ...
                    for (var i = 0; i < properties.length; i++) {
                        ...
                        // First we check if there is a config for 'key-type'
						// and then for 'type' alone
            ...
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                ...
                if (shapes && shapes.length == 1) {
        			...
        			if (morphShapes && morphShapes.length > 0) {
        				// In case the element is not wide enough, start the 2
						// bottom-buttons more to the left
            ...
            if (!$rootScope.stencilInitialized) {
	            ...
	            KISBPM.eventBus.addListener(KISBPM.eventBus.EVENT_TYPE_PROPERTY_VALUE_CHANGED, function (event) {
	                if (event.property && event.property.key) {
	                    // If the name property is been updated, we also need to
						// change the title of the currently selected item
            ...
            $scope.quickAddItem = function(newItemId) {
            	$scope.safeApply(function () {
            		...
            		if (shapes && shapes.length == 1)
            		{
            			...
            			if (!targetStencil){ return; }// Check if there can be
														// a target shape
        ...
        /*
		 * Method available to all sub controllers (for property controllers) to
		 * update the internal Oryx model
		 */
        $scope.updatePropertyInModel = function (property, shapeId) {

            var shape = $scope.selectedShape;
            // Some updates may happen when selected shape is already changed,
			// so when an additional
            // shapeId is supplied, we need to make sure the correct shape is
			// updated (current or previous)
            if (shapeId) {
            ...
            if (!shape) {
                // When no shape is selected, or no shape is found for the
				// alternative
                // shape ID, do nothing
                return;
            }
        ...
        /**
		 * Helper method that searches a group for an item with the given id. If
            ...
            if ($scope.dragCanContain) {
            	...
            	if ($scope.quickMenu)
            	{
            		...
            		if (shapes && shapes.length == 1)
            		{
	        			        ...
	        			        containedStencil.idWithoutNs() !== 'MessageFlow' && containedStencil.idWithoutNs() !== 'DataAssociation')
	        			{
		        			...
		        			var targetStencil = $scope.editor.getRules().connectMorph(args);
		        			if (!targetStencil){ return; }// Check if there
															// can be a target
															// shape
		        			option.connectingType = targetStencil.id();
	        			}
            	...
            	else
            	{
                    ...
                    if (containedStencil.idWithoutNs() === 'BoundaryErrorEvent' || containedStencil.idWithoutNs() === 'BoundaryTimerEvent' ||
                        ...
                        // Modify position, otherwise boundary event will get
						// position related to left corner of the canvas instead
						// of the container
            ...
            if ($scope.dragModeOver != false) {
                ...
                else 
                {
                    ...
                    if (item.type === "node") {
                        ...
                        // check if the draggable is a boundary event and the
						// parent an Activity
