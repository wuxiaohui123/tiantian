...
<%--@doc--%>
<%@tag description='可选择label文本框' display-name="selectLableText" %>
...
<div style="position: relative;height:28px; float:left;<%if(labelWidth!=null){%>width:<%=labelWidth%>px;<%}else{%>width:100px;<%}%>">
	<div class="selectLabelText_label" onclick="fnSelectLabelTextLabelClick_<%=id%>(this)">
	...
	%>
	<div onclick="fnSelectLabelClick_<%=id%>(this)" _id="<%=list.get(i).get("id")%>"><%=list.get(i).get("key") %></div>
	<%
	}
	 %>
	 <script>
	 	function fnSelectLabelClick_<%=id%>(o){
	 		var $input = $(o).parent().parent().next().children("input");
	 		var $inputid = $input.attr("id");
	 		var $tempid = $(o).attr("_id");
	 		var temp = $(o).html();
	 		var labeltemp = $("#selectLabelText_label_span_<%=id%>").html();
	 		$("#selectLabelText_label_span_<%=id%>").html(temp);
	 		$(o).html(labeltemp).attr("_id",$inputid);
	 		$input.attr("id",$tempid); 
	 		$input.attr("name","dto['"+$tempid+"']");
	 		$(o).parent().hide();
	 	}
	 	function fnSelectLabelTextLabelClick_<%=id%>(o){
	 		$(o).next().css("left",$(o).position().left).show();
	 	}
	 	$(function(){
	 		$("body").bind("mousedown",function(e){
	 			var target = e.target || e.srcElement; 
    			 if(!$(target).parents().is(".selectLabelText_C")){
		 			$(".selectLabelText_C").hide();
    			 }
	 		});
	 	});
	 </script>
